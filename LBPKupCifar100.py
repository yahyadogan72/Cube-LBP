...
model_num = 'Orj'
log_file = './outputs100/log_{}.txt'.format(model_num)
...
def plot_confusion_matrix(modelnum, cm, classes, normalize=False, title='Confusion Matrix'):
    ...
    plt.savefig('./outputs100/{}.png'.format(modelnum), dpi=300)
...
def modelgetir(inputsize):
    ...
    # model.add(tf.keras.applications.DenseNet201(include_top=False, weights="imagenet",input_shape=input_shape))
...
def calculate_lbp(patchR, patchG, patchB,durum=0):
    ...
    if durum==0:
        ...
        s=tf.cast(tf.stack([lbp1,lbp2,lbp7], axis=-1), dtype=tf.float32)
        
        #(tf.cast(s, tf.float32) / 127.5) - 1.0
    ...
    s=tf.cast(tf.stack([lbp1,lbp2, lbp3, lbp4, lbp5, lbp6], axis=-1), dtype=tf.float32)  
...
def lbp_from_tensor(img_tensor,durum=0):
    ...
    tum = tf.concat([img_tensor/255, lbp_matrix], axis=-1)
...
weight_path = "{}.hdf5".format('lbpKupOrj100')
...
for i in range(0,4):   
    ...
    reduceLROnPlat = tf.keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=7, verbose=1, mode='min')
    callbacks_list = [reduceLROnPlat,checkpoint]
    modelcustom.append(model.fit(x_train_processed, y_train, batch_size=32,
                            epochs=epochs,
                            verbose=1,
                            shuffle=False,
...
model=modelgetir(3)
modeln = model.fit(x_train1, y_train, batch_size=32,
                        epochs=epochs,
                        verbose=1,
                        shuffle=False,
